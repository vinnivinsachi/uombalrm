{if $messages|@count>0}
	...
	{/if}
{get_tag_summary user_id=$identity->getId() object="post_cat" assign=summary}
...
{if $summary|@count>0}
	<div id="preview-tags" class="box">
		...
		<ul>
			...
			<li>
				<a href="{geturl controller='blogmanager'}?cat={$tag.tag}">
				{$tag.tag|escape}
				</a>
...
{get_tag_summary user_id=$identity->getId() object="post" assign=summary}

{if $summary|@count>0}
	<div id="preview-tags2" class="box">
		<h3>Your Post Tags/Page</h3>
		<ul>
			{foreach from=$summary item=tag}
			<li>
...
{get_monthly_blogs_summary user_id=$identity->getId() object="post" assign=summary}
