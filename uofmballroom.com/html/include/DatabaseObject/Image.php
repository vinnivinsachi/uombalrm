...
//from uofmballroom
	...
	class DatabaseObject_Image extends DatabaseObject
	{
		...
		public function __construct($db)
		{
			...
			{
			$this->databaseColumn = 'shoutout_id';
			$this->table = 'shout_out_image';
			parent::__construct($db, 'shout_out_image', 'image_id');
			}
		...
		public function preInsert()
		{
			...
			{
				$query = sprintf('select coalesce(max(ranking),0) + 1 from %s where '.$this->databaseColumn.' = %d', $this->_table, $this->shoutout_id);
			}
		...
		public function createThumbnail($maxW, $maxH, $thumbType='')
		{
			...
			if($maxW==0 && $maxH!=0)
			{
				$filename = sprintf('%d.H%d_%s.jpg', $this->getId(), $maxH, $thumbType);
			}
			elseif($maxH==0 && $maxW!=0)
			{
				$filename = sprintf('%d.W%d_%s.jpg', $this->getId(), $maxW, $thumbType);
			}
		...
		{
			return sprintf('%s/%d.jpg', self::GetUploadPath(), $this->getId());
		}
